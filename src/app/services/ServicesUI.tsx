...
import { getServicePageData } from "@/services/APIServices";
import { AccordionData, DocumentData, SectionData } from "@/types/common";
import Link from "next/link";

// Helper Accordion Panel with animation using maxHeight
function AnimatedAccordionPanel({
  ...
  useEffect(() => {
    ...
    if (el) {
      if (expanded) {
        ...
        // Need to let display:block render before maxHeight in next tick
        requestAnimationFrame(() => {
          el.style.maxHeight = el.scrollHeight + "px";
        });
      } else {
        el.style.maxHeight = el.scrollHeight + "px";
        // in the next tick, set maxHeight to 0 to trigger transition
        requestAnimationFrame(() => {
  ...
  // On transition end, hide if collapsed
  ...
  return (
    <div
      ...
      aria-labelledby={`heading_${panelId}`}
      data-bs-parent="#accProductDetail"
      style={{
        overflow: "hidden",
...
// Fix: Custom React accordion functionality with animation
export default function ServicesUI({ slug }: { slug: string }) {
  const [data, setData] = useState<SectionData[]>([]);
  const [activeData, setActiveData] = useState<SectionData | null>(null);
  const [openAccordionIndex, setOpenAccordionIndex] = useState<number | null>(0);

  useEffect(() => {
    getServicePageData().then((data: SectionData[]) => {
      setData(data);
      const found = data.find((item: SectionData) => item.slug === slug) || null;
      setActiveData(found);
      setOpenAccordionIndex(0); // reset open to first on slug/data change
    });
  }, [slug]);
  ...
  return (
    <div className="ir-wrapper">
      ...
      <div className="section-ptb services-overview ir-scroll-container">
        <div className="ir-container">
          <div className="row gx-xl-5">
            <div className="col-lg-auto col-content-left">
              <div className="content-left">
                <div className="ir-nav-left">
                  ...
                  <ul>
                    {data?.map((item: SectionData, index: number) => (
                      <li
                        key={index}
                        className={item.slug === activeData?.slug ? "active" : ""}
                      >
                        <Link
            ...
            <div className="col-lg col-content-right">
              <div className="content-right">
                ...
                {/* Custom Accordion (JS, not bootstrap) */}
                <div className="services-accordion">
                  <div
                    className="accordion ir-accordion acc-secondary"
                    id="accProductDetail"
                  >
                    {activeData?.Accordion?.map(
                      (item: AccordionData, index: number) => (
      ...
      <div className="services-media-seminars bg-grey pattern-bottom">
        {/* <EventNSeminar /> */}
      </div>
      <div className="section-ptb other-services">
        <div className="ir-container">
          ...
          <div className="other-services-wrapper">
            <div className="row">
              {data
                .filter((item: SectionData) => item.slug !== activeData?.slug)
                .map((item: SectionData, index: number) => (
                  <div key={index} className="col-xl-6 col-os-card">
                    <div className="os-card">
                      <div className="row gx-0 align-items-center">
                        <div className="col-sm-auto col-thumb">
                          <div className="os-thumb">
                            <img
                              className="thumb"
                              src={
                                item?.ListingImage?.url ||
                                "/images/financial-sector.jpg"
                              }
                              alt={item.Title}
                            />
                            <img
                        ...
                        <div className="col-sm">
                          <div className="os-caption">
                            ...
                            <div className="os-cta">
                              <Link href={`/services/${item.slug}`} className="btn-ir-cta blue">
                                <svg
                                  width="14"
                                  height="14"
                                  viewBox="0 0 14 14"
