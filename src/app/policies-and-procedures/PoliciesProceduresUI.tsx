...
import { getPoliciesAndProceduresData } from "@/services/APIServices";
...
const CorporateGovernanceUI = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await getPoliciesAndProceduresData();
          ...
          <div className="articles-row">
            <div className="row">
              {data.map((item) => (
                <div
                  key={item.id}
                  className="col-xl-4 col-md-6 col-articles"
                >
                  <Link
                  className="article-card"
                  href={`${item.slug==="fa-qs-on-ratings"?"/ratings/rating-faqs":"/policies-and-procedures/"+item.slug}`}
